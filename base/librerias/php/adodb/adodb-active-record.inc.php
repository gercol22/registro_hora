...
class ADODB_Active_Record {
	...
	/*MODIFICACION DEL METODO ELIMINAR SE AGREGO PARAMETRO
	 * PARA INDICAR SI EL OBJETO LE FUE SETEADO UN FILTRO
	 * DE BUSQUEDA EL CUAL SUPLANTA LA CLAVE PRIMARIA OBTENIDA
	 * POR LA LIBRERIA
	 * */
	function Delete($load = false)
	{
		$db = $this->DB(); if (!$db) return false;
		
		//AQUI CAMBIO DEL PK POR _WHERE 		
		if (!$load) {
			$table = $this->TableInfo();
			$where = $this->GenWhere($db,$table);
		}
